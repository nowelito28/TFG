...
Su técnica más habitual de intrusión consiste en explotar el mecanismo de enlazado dinámico \texttt{LD\_PRELOAD} para cargar bibliotecas maliciosas e interceptar funciones (\textit{hooking}) de bibliotecas típicamente utilizadas en C (como \texttt{libc}), modificando la salida de comandos críticos como \texttt{ps}, \texttt{ls}, \texttt{top} o \texttt{netstat}.
Esta capacidad de manipulación permite al atacante occultar:
...
En este contexto, el proyecto se centra en diseñar un mecanismo de reporte seguro que permita al administrador obtener información crítica del sistema desde el \textit{Espacio del Kernel}, acompañado de un mecanismo de autenticación criptográfica que permita verificar su integridad y autenticidad posteriormente desde el \textit{Espacio de Usuario}, incluso en escenarios comprometidos por \textit{rootkits} de \textit{Espacio de Usuario}.
