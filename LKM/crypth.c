...
// Mutex para proteger acceso a fd_glob (handlers /proc pueden dormir)
static DEFINE_MUTEX (mtx_fd);
...
static int
write_full (struct file *f, const char *buf, int len)
{
  int off = 0;
  int w;
  loff_t *ppos = &f->f_pos;
...
static int
write_cont_hmac (struct file *f, const char *cont, int cont_len,
...
static int
get_hmac_sha256 (const u8 *buf, int buf_len, u8 **hmac,
		      int *hmac_len)
{
  int rc;
...
static int
get_hmac_b64 (const u8 *hmac, int hmac_len, char **hmac_b64,
    int *hmac_b64len)
{
...
// Función para certificar el contenido que queremos poner 
// en el fichero dado por 'fd' con HMAC(SHA-256) con clave K
...
printH (int fd)
{
  ...
  if (fsize != 0)
    {
      printk (KERN_ERR
	      "Error printH: fd %d must refer to an empty file (size=%lld)\n",
	      fd, fsize);
      rc = -ENOTEMPTY;
      goto out_put;
    }
  ...
  rc = get_hmac_sha256 (cont, cont_len, &hmac, &hmac_len);
  if (rc < 0)
    {
      printk (KERN_ERR
	      "Error printH: generating HMAC failed for fd %d: %zd\n", fd,
	      rc);
      goto out_put;
    }
  
  // 5) Pasar el HMAC a Base64:
  rc = get_hmac_b64 (hmac, hmac_len, &hmac_b64, &hmac_b64len);
  if (rc < 0)
    {
      printk (KERN_ERR
	      "Error printH: parsing HMAC to Base 64: %zd\n", rc);
      goto out_free_hmac;
    }

  // 6) Escribir contenido y HMAC en el fichero fd -> cont + sep + HMAC(base 64):
	 ...
	 loff_t *ppos)
{
  ...
  *ppos = c;
  ...
  // -> kstrtoint(char[], base, &res) -> y cambiar valor de fd_glob (protegido)
  ...
  printk (KERN_DEBUG
	  "mywrite: printH OK (content certificated by HMAC(SHA-256)) for fd %d (%zd bytes written)\n",
	  fd, written);
...
myread (struct file *file, char __user *ubuf, size_t count, loff_t *ppos)
{
  ...
  if (*ppos > 0 || count < BUFSIZE)
    {
      printk (KERN_ERR
	      "/proc/fddev: Only one read allowed or too much bytes requested (100 bytes max)\n");
      return 0;
    }
  printk (KERN_DEBUG "/proc/fddev: read handler\n");

  // 2) Escribe contenido solicitado por userspace -> sprintf(destino, formato, valores…)
  // -> fd_glob (protegido) -> último fd/canal utilizado
  mutex_lock (&mtx_fd);
  fd = fd_glob;
  mutex_unlock (&mtx_fd);
